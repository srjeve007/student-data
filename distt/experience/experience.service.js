...
let ExperienceService = class ExperienceService {
    ...
    async createExperience(createExperienceDto, userCredentials) {
        ...
        if (createExperience && createExperience.length) {
            ...
            for (const createExperience_ of createExperience) {
                ...
                if (!experienceExists) {
                    ...
                    throw new common_1.NotFoundException(`Experience with name "${Profile}" already exists`);
                }
            }
            return are;
        }
    }
    async getExperience(filterExperienceDto, userCredentials) {
    ...
    async getExperienceById(id, userCredentials) {
    ...
    async deleteExperienceById(id, userCredentials) {
        const found = await this.experienceRepository.findOne({ where: { id } });
        if (found) {
    ...
    async updateExperience(id, updateExperienceDto, userCredentials) {
